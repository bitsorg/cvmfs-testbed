...
# ── 3c. Wait for the repository to exist ──────────────────────────────────────
# Under `make` init finishes mkfs before this container starts, but a bare
# `docker compose up` (or restart) launches bootstrap concurrently with init.
# The gateway grants the lease regardless (its repo.json is static), and the
# ingest then dies fetching .cvmfspublished from a bucket mkfs has not written
# yet: "Error opening file: No such file or directory / Impossible to start a
# transaction". Observed three times on 2026-08-12, losing to mkfs by ~10 s
# each time. Poll the same manifest URL cvmfs_server will read, from the same
# server.conf, so the wait cannot check a different store than the ingest uses.
# server.conf itself is generated by init post-mkfs, so on a fresh concurrent
# start it appears at roughly the same moment as the manifest — it must be
# part of the poll, not a precondition (the bind-mount is a directory; files
# written by init become visible inside the running container).
_server_conf="/etc/cvmfs/repositories.d/${REPO_NAME}/server.conf"
_manifest_url=""
_last_poll_err=""
_manifest_ready() {
    [[ -f "$_server_conf" ]] || { _last_poll_err="no ${_server_conf} yet"; return 1; }
    local s0
    s0=$(sed -n 's/^CVMFS_STRATUM0=//p' "$_server_conf" | tail -1)
    [[ -n "$s0" ]] || { _last_poll_err="CVMFS_STRATUM0 not set in ${_server_conf}"; return 1; }
    _manifest_url="${s0}/.cvmfspublished"
    _last_poll_err=$(curl -fs --max-time 5 -o /dev/null "$_manifest_url" 2>&1) || {
        _last_poll_err="curl ${_manifest_url}: ${_last_poll_err:-HTTP error}"; return 1; }
    return 0
}
_wait_secs="${BOOTSTRAP_WAIT_REPO_SECS:-120}"
[[ "$_wait_secs" =~ ^[0-9]+$ ]] || { warn "BOOTSTRAP_WAIT_REPO_SECS='${_wait_secs}' is not a number; using 120."; _wait_secs=120; }
info "Waiting up to ${_wait_secs}s for the repository (server.conf + manifest) ..."
_deadline=$((SECONDS + _wait_secs))
until _manifest_ready; do
    if (( SECONDS >= _deadline )); then
        die "repository never became ready (waited ${_wait_secs}s)." \
            "\n       last check: ${_last_poll_err}" \
            "\n       init has not completed (or failed) — check the init step;" \
            "\n       raising BOOTSTRAP_WAIT_REPO_SECS only helps if init is" \
            "\n       genuinely still running."
    fi
    sleep 2
done
success "Repository ready — manifest answers at ${_manifest_url}."
